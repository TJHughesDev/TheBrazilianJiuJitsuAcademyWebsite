...
(function () {
  ...
  function typeBadge(type) {
    if (type === "Kids")
      return '<span class="schedule-card__badge badge--kids">Kids</span>';
    if (type === "Adults")
      return '<span class="schedule-card__badge badge--adults">Adults</span>';
  ...
  function renderSchedule(sessions) {
    if (!sessions || !sessions.length) {
      scheduleGrid.innerHTML =
        '<p style="color:var(--grey-3);padding:32px 0;">No sessions listed yet.</p>';
      return;
    }
    ...
    var html = sessions
      .map(function (s) {
        return (
          ...
          '<span class="schedule-card__day-abbr">' +
          (DAY_ABBR[s.day] || s.day.slice(0, 3).toUpperCase()) +
          "</span>" +
          "</div>" +
  ...
  fetch("/data/schedule.json")
    ...
    .catch(function () {
      scheduleGrid.innerHTML =
        '<p style="color:var(--grey-3);padding:32px 0;">Could not load schedule. Please contact us directly.</p>';
    });
  ...
  function renderInstructors(instructors) {
    if (!instructors || !instructors.length) {
      instructorsGrid.innerHTML =
        '<p style="color:var(--grey-3);padding:32px 0;">No instructors listed yet.</p>';
      return;
    }

    var html = instructors
      .map(function (i) {
        var beltClass = BELT_CLASS[i.belt] || "belt--black";
        var lineageHtml = i.lineage
          ? '<div class="instructor-card__lineage"><strong>Lineage</strong>' +
            escHtml(i.lineage) +
            "</div>"
          : "";
        ...
        return (
          ...
          (i.image
            ...
            : initials(i.name)) +
          "</div>" +
          '<span class="instructor-card__belt ' +
          beltClass +
          '">' +
          escHtml(i.belt) +
          "</span>" +
          ...
          escHtml(i.name) +
          "</div>" +
          '<div class="instructor-card__title">' +
          escHtml(i.title) +
          "</div>" +
  ...
  fetch("/data/instructors.json")
    ...
    .catch(function () {
      instructorsGrid.innerHTML =
        '<p style="color:var(--grey-3);padding:32px 0;">Could not load instructor information.</p>';
    });
